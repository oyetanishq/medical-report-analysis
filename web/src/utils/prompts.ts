import { PatientData } from "@/pages/home";

export const patientSummarise = (reportContent: string, patientData: PatientData) => {
    return `Role: You are an expert Medical Data Analyst AI.
...
Recent Symptoms: ${patientData.symptoms || "None reported"}

[BIOMARKER TABLE]:
Please analyze the medical report above and provide a comprehensive medical analysis.`;
};

export const getBioMakersDetails = (reportContent: string, patientData: PatientData) => {
    return `Role: You are an expert AI Health Advisor. Your purpose is to analyze patient medical data and present it in a clear, structured, and actionable format. You are NOT a doctor, and all your advice is for informational purposes and should be reviewed by a qualified medical professional.
